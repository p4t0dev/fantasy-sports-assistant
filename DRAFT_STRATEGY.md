...
* **The "Fake" Superflex Trap:** The `assistant.py` tool prints the starter requirements. If a league has `SUPER_FLEX=1` but `QB=0`, **it is not a Superflex league.** It is a 1-QB league in disguise because you can only start a maximum of one Quarterback total. In this format, the positional value of QBs is drastically reduced. Do NOT stash developmental QBs or reach for bridge veterans. Treat it strictly as a 1-QB format.
